...
/** خروجی: لیست انتقال‌ها با کمترین تعداد تراکنش (حریصانه) */
...
export default function SettlementsPage({ params }: { params: { id: string } }) {
  ...
  const netsList = useMemo(() => {
    ...
    // فقط کسایی که تراکنش داشتن رو هم نگه داریم
    return list.filter(x => x.net !== 0);
  }, [players, netsMap]);

  const transfers = useMemo(() => computeSettlements(netsList), [netsList]);

  if (loading) return <div className="container"><h1>Settlements</h1><p className="label-muted">Loading…</p></div>;
  if (err) return <div className="container"><h1>Settlements</h1><p className="label-muted">Error: {err}</p></div>;
